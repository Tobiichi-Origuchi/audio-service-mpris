...
Future<void> main() async {
  final Mpris mpris = AudioServiceMpris.init(
            ...
            Row(
              children: [
                StreamBuilder<AudioServiceRepeatMode>(
                  stream: _audioHandler.playbackState.map((state) => state.repeatMode).distinct(),
                  builder: (context, snapshot) {
...
class AudioPlayerHandlerImpl extends BaseAudioHandler
  ...
  Future<void> _init() async {
    ...
    _effectiveSequence
        .map((sequence) => sequence.map((source) => _mediaItemExpando[source]!).toList())
        .pipe(queue);
    // Load the playlist.
  ...
  Future<void> moveQueueItem(int currentIndex, int newIndex) async {
    final one = _playlist[newIndex];
    _playlist[newIndex] = _playlist[currentIndex];
    _playlist[currentIndex] = one;
  }
